...
//
// This declaration file is the single source of truth. It is hand-maintained,
// not generated - do not add a types.ts alongside it, and do not point tsc at
// this directory to emit into it.
//
// Why a .d.ts rather than a .ts: the backend compiles with rootDir ./src, so a
// .ts file outside that directory fails with TS6059 and cannot be imported. A
// declaration file has no emit, so it crosses the boundary cleanly. Every
// consumer imports from here with `import type`, so nothing survives to
// runtime on either side.
//
// This works only while these types stay declaration-only. Adding a runtime
// value (a const, enum, or function) means turning shared/ into a real package
// that is built before its consumers.
